...
Nodes:
   ...
   stop_app                      Stops the RabbitMQ application, leaving the runtme (Erlang VM) running
...
Cluster:
   ...
   update_cluster_nodes          Instructs an already clustered node to contact <clusternode> to cluster when waking up
...
Users:
   ...
   clear_password                Removes the user password
   delete_user                   Removes the specified user
...
Access Control:
   ...
   list_user_permissions         Lists user permissions
...
Monitoring, observability and health checks:
   ...
   node_health_check             Performs several health checks of the target node
...
Virtual hosts:
   ...
   restart_vhost                 Restarts a failed vhost data storage and queues
...
Node configuration:
   ...
   set_cluster_name              Sets the cluster name to name
