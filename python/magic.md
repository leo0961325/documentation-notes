...
## `__get__()`
- object 若有綁定 `__get__(self, obj, type=None)`, 則此物件稱之為 `Descriptor(描述器)` / `Non-Data Descriptor(非資料描述器)`
    - self 為 Descriptor instance
    - obj  為 Descriptor 所 attached to 的 Object
    - type 為 Descriptor 所 attached to 的 Object
- 若使用 a.x, 首先會先尋找 `a.__dict__['x']`, 再者尋找 `type(a).__dict__['x']`(父類別們)
- 如果 instance.attr1 與 `instance.__dict__['attr1']` 同名, 則**後者**優先
    - Non-Data Descriptors, 

## `__set__()`
- object 若有綁定 `__set__(self, obj, vlaue)`, 則此物件稱之為 `Descriptor(描述器)` / `覆寫式描述器(Overriding Descriptor)` / `Data Descriptor(資料描述器)`
...
## `__delete__()`
- object 若有綁定 `__delete__()`, 則此物件稱之為 `Descriptor(描述器)` / `覆寫式描述器(Overriding Descriptor)` / `Data Descriptor(資料描述器)`
- 如果 instance.attr1 與 `instance.__dict__['attr1']` 同名, 則**前者**優先
