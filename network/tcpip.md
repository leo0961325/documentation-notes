...
(一個 Network ID 有 32-2 台)
第一組分配給 網域y
192.168.0.64/27 分配給 28 台電腦的 Subnet, 可用 ip 區間為
192.168.0.[65~95] (只浪費 2 個 ip)

之後, 再把剩下的 192.168.0.011|00000 繼續借用 bit

借用 4個 bit
192.168.0.0110|0000 -> 192.168.0.96/27
192.168.0.0111|0000 -> 192.168.0.112/27 (這後面的東西, 這次用不到了~~ 保留下來將來用)
(一個 Network ID 有 16-2 台)

借用 5個 bit
192.168.0.01100|000 -> 192.168.0.96/28
192.168.0.01101|000 -> 192.168.0.104/28
(一個 Network ID 有 8-2 台)
第一組分配給 網域z
192.168.0.96/28 分配給 6 台電腦的 Subnet, 可用 ip 區間為
192.168.0.[97~103] (沒有浪費 ip)

------------- 結果 -------------
網域x 有 60 台電腦 192.168.0.[1~62]  , 浪費 2 個 ip
網域y 有 28 台電腦 192.168.0.[65~95] , 浪費 0 個 ip
網域z 有  6 台電腦 192.168.0.[97~103], 浪費 2 個 ip
```
